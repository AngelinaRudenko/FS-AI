...
\usepackage{setspace}
\usepackage{enumitem}
\usepackage{float} % for fixing floating images
% \usepackage{pdfpages}
...
\href{https://colab.research.google.com/drive/14Ko9NzpKZGfawO7YVp_15a5EEHjBnkYC?usp=sharing}{Google Colab Link}

\href{https://github.com/AngelinaRudenko/FS-AI}{GitHub Link}
...
\begin{itemize}[leftmargin=*]
    \item It's large enough to train a model.
    \item It contains real flight data.
    \item The features are easy to understand
\end{itemize}

This documentation is organized as follows:

\begin{itemize}[leftmargin=*]
    \item Section 2 - Dataset Description: Details about the data, including features and basic statistics.
    \item Section 3 - Problem Definition: Explanation of the prediction task and chosen machine learning method.
    \item Section 4 - Data Preprocessing: How I cleaned and prepared the data for machine learning.
    \item Section 5 - Machine Learning Method: Description of the Random Forest algorithm and its parameters.
    \item Section 6 - Experiments and Results: Describes machine learning models I built, how I evaluated them, and the final results.
    \item Section 7 - Discussion: What worked well, what didn't work, and ideas for future improvements.
    \item Section 8 - Conclusion: Summary of the project and key findings
    \item Appendices: Additional plots and data profiles used in the analysis.
\end{itemize}
...
\noindent\textbf{Strengths of the Model}

\noindent\textit{High Accuracy.} The final model achieved an R² score of 98.56\%, which means it can explain more than 98\% of the variation in flight prices. This is a good result.

\noindent\textit{No Overfitting.} The difference between training performance (R² = 99.32\%) and testing performance (R² = 98.56\%) is very small. This shows that the model learned real patterns instead of just memorizing the training data.

\medskip
\noindent\textbf{Weaknesses of the Model}

\noindent\textit{One Feature Dominates.} The class feature has 82\% importance, which means the model relies heavily on just one piece of information. I expected departure and arrival times and cities to be more important. The dataset might not have enough variation in prices by time. Business class is expensive regardless of whether the flight departs in the morning or evening.

\noindent\textit{Larger Errors for Expensive Flights.} Looking at the residual plot, I noticed that errors are bigger for expensive flights. The model is less accurate when predicting high prices. This happened because there are fewer expensive flights in the dataset (most flights are economy class). The model has less training data for expensive flights, so it makes bigger errors for them.

\medskip
\noindent\textbf{What Worked Well and Why}

\noindent\textit{Random Forest Algorithm.} Random Forest worked well for this problem. It handled different types of features (numbers and categories) without problems and didn't need much tuning. It worked because Random Forest can capture complex relationships (for example, that business class flights to certain cities are more expensive), it is robust and doesn't overfit easily, and it works well with categorical features like airline and city names.

\noindent\textit{Label Encoding.} I used Label Encoding to convert categorical features (like airline names) into numbers. This worked well and Random Forest could handle this type of encoding.

\medskip
\noindent\textbf{Possible Alternatives}

\noindent Instead of one model for all flights, I could create two separate models: one for economy class flights and one for business class flights. This might help because each model could focus on the price variations within that class.

% --------------------------------------------------------------------

\section{Conclusion}

Overall, I'm satisfied with the results of this project. The Random Forest model achieved good accuracy (98.56\% R²) and works reliably on new data. The most important finding is that flight class dominates price prediction in the Indian market and this one factor explains 82\% of price variation.
...
\begin{itemize}[leftmargin=*]
    \item Real-world data preprocessing.
    \item Feature engineering.
    \item Model evaluation.
    \item Interpreting visualizations to understand model behavior.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/numerical-features-distribution.png}
...
\begin{itemize}
    \item brainstorming which topic to choose,
